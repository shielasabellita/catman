...
frappe.ui.form.on("Rename Tool", {
	onload: function(frm) {
		return frappe.call({
			method: "category_management.category_management.doctype.rename_tool.rename_tool.get_doctypes",
			callback: function(r) {
	...
	refresh: function(frm) {
		...
		frm.page.set_primary_action(__("Rename"), function() {
			...
			frappe.call({
				method: "category_management.category_management.doctype.rename_tool.rename_tool.upload",
				args: {
					select_doctype: frm.doc.select_doctype,
					merge:frm.doc.merge_existing
				},
				callback: function(r) {
